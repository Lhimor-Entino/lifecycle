...
class HandleInertiaRequests extends Middleware
{
    ...
    public function share(Request $request)
    {
        ...
        if ($request->user()){
            ...
            $projects = $company_id!='LS50'? Project::with(['programs'])->where('is_archived',0)->where(function ($q) use($ids,$user_id){
                $q->whereIn('id',$ids);
                $q->orWhere('user_id',$user_id);
                // $q->orWhere('user_id',$user_id);
            })->get():Project::with(['programs'])->where('is_archived',0)->get();   
        
 
            $active_bus_doc_req_ids = BusReqDoc::where("is_active",0)->get()->pluck('id','program_id')->toArray();
            ...
            $acknowledge_programs = Project::with(['acknowledge_programs'])->where('is_archived',0)->get();
