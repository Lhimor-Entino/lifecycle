...
import { ChangeManagementRequest, HrmsInfo, PageProps, Program, User } from '@/types';
import { useForm, usePage } from '@inertiajs/inertia-react';
import { useQuery } from '@tanstack/react-query';
import axios from 'axios';
import { parse,format, formatDate } from 'date-fns';
...
const EditChangeManagementRequest = () => {
    const {program} = useProgramModal();
    ...
    const {isOpen,onClose,cmr_id} = useChangeManagementModal();
    const [currentHead,setCurrentHead] = useState<string|undefined>("")
    const [currentComBy,setCurrentComBy] = useState<string|undefined>("")
    const [currentNotedBy,setCurrentNotedBy] = useState<string|undefined>("")
    ...
    const onSubmit:FormEventHandler<HTMLFormElement> = e =>{
        ...
        if (!selected_program) return;
            ...
            const href = route('changeMngt.update',{program_id: selected_program.id})
        ...
        useEffect(() => {
            ...
            const toEdit:ChangeManagementRequest[] = (selected_program.change_requests.filter(s_id =>  s_id.id === cmr_id  ))
            console.log(toEdit)
            ...
            const {
                ...
                description,
                client_request,
                incident_or_problem_resolution,
                enhancement,
                ...
                risk_impact_analysis_schedule,
                risk_impact_analysis_equipment,
                risk_impact_analysis_overall_impact,
                rollback_plan,
                review_result,
                reason,
                head,
                ...
                completed_by_date,
                noted_by,
                noted_by_date} = toEdit[0]

                
            setCurrentHead(head?.first_name + " " + head?.last_name)
            setCurrentComBy(completed_by?.first_name + " " + completed_by?.last_name )
            setCurrentNotedBy(noted_by?.first_name + " " + noted_by?.last_name)
           
            const formattedHead = { 
                job_job_title : head?.position || "",
                idno : head?.company_id ||"",
                last_name :head?.last_name ||"",
                first_name : head?.first_name ||"",
                picture_location : head?.photo ||"",
                department : head?.department ||"",
                work_email : head?.email||""
            }
            const formattedCompletedBy = { 
                job_job_title : completed_by?.position || "",
                idno : completed_by?.company_id ||"",
                last_name :completed_by?.last_name ||"",
                first_name : completed_by?.first_name ||"",
                picture_location : completed_by?.photo ||"",
                department : completed_by?.department ||"",
                work_email : completed_by?.email||""
            }
            const formattedNoteddBy = { 
                job_job_title : noted_by?.position || "",
                idno : noted_by?.company_id ||"",
                last_name :noted_by?.last_name ||"",
                first_name : noted_by?.first_name ||"",
                picture_location : noted_by?.photo ||"",
                department : noted_by?.department ||"",
                work_email : noted_by?.email||""
            }
            setData(val=>({...val,
                id,
                title,
                ...
                other : other || "",
                risk_impact_analysis_people: risk_impact_analysis_people || "",
                risk_impact_analysis_affected_system : risk_impact_analysis_affected_system || "",
                risk_impact_analysis_schedule: risk_impact_analysis_schedule || "",
                risk_impact_analysis_equipment: risk_impact_analysis_equipment || "",
                risk_impact_analysis_overall_impact: risk_impact_analysis_overall_impact || "low",
                rollback_plan:rollback_plan ||"",
                review_result:review_result || 1,
                reason:reason||"",
                head: formattedHead,
                ...
                remarks : remarks || "",
                completed_by: formattedCompletedBy,
                completed_by_date : completed_by_date || "",
                noted_by: formattedNoteddBy,
                noted_by_date: noted_by_date || ""
             }));
    ...
    return (
        <Dialog modal open={isOpen} onOpenChange={onClose}>
            <DialogContent className='max-w-[90vw] max-h-[95vh] h-full flex flex-col space-x-1.5'>
                ...
                <form onSubmit={onSubmit} className='flex flex-col gap-y-3.5 flex-1 overflow-y-auto relative pr-6 pb-6'>
                            ...
                            <Popover open={openUsers} onOpenChange={setOpenUsers}>
                                <PopoverTrigger asChild>
                                    <Button disabled={processing} variant="outline" role="combobox" aria-expanded={openUsers} className="w-full justify-between" >
                                        {!data.head? currentHead : `${data.head.first_name} ${data.head.last_name}`}
                        ...
                        <div className='space-y-1'>
                            ...
                            <Popover>
                                <PopoverTrigger asChild>
                                    ...
                                    >
                                    <CalendarIcon className="mr-2 h-4 w-4" />
                                    {data.review_date ? format(new Date(data.review_date), "PPP") : <span>Pick a date</span>}
                                    </Button>
                                </PopoverTrigger>
                                <PopoverContent className="w-auto p-0">
                                    <Calendar
                                    ...
                                    onSelect={e=>e&&setData('review_date',format(e,'yyyy-MM-dd'))}
                                    initialFocus
                                    />
                                </PopoverContent>
                            </Popover>
                        ...
                        <div className='space-y-1'>
                            ...
                            <Popover>
                                ...
                                <PopoverContent className="w-auto p-0">
                                    ...
                                    onSelect={e=>e&&setData('actual_start',format(e,'yyyy-MM-dd'))}
                                    initialFocus
                                    />
                                </PopoverContent>
                            </Popover>
                        ...
                        <div className='space-y-1'>
                            ...
                            <Popover>
                                ...
                                <PopoverContent className="w-auto p-0">
                                    ...
                                    onSelect={e=>e&&setData('actual_end',format(e,'yyyy-MM-dd'))}
                                    initialFocus
                                    />
                                </PopoverContent>
                            </Popover>
                            ...
                            <Popover open={openCompletedBy} onOpenChange={setOpenCompletedBy}>
                                <PopoverTrigger asChild>
                                    <Button disabled={processing} variant="outline" role="combobox" aria-expanded={openCompletedBy} className="w-full justify-between" >
                                        {!data.completed_by?currentComBy : `${data.completed_by.first_name} ${data.completed_by.last_name}`}
                                        <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                    </Button>
                                </PopoverTrigger>
                                <PopoverContent className="w-full p-3.5 !z-[700]">
                            ...
                            <Popover open={openNotedBy} onOpenChange={setOpenNotedBy}>
                                <PopoverTrigger asChild>
                                    <Button disabled={processing} variant="outline" role="combobox" aria-expanded={openNotedBy} className="w-full justify-between" >
                                        {!data.noted_by?currentNotedBy: `${data.noted_by.first_name} ${data.noted_by.last_name}`}
                                        <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                    </Button>
                                </PopoverTrigger>
                                <PopoverContent className="w-full p-3.5 !z-[700]">
                    ...
                    <Button disabled={processing} size='sm' className='mt-[3.5rem] p-2 font-boldbg-slate-300text-slate-950hover:bg-slate-700 text-slate-100 bg-slate-700 dark:bg-slate-100
                    dark:hover:bg-slate-700 dark:text-slate-700 dark:hover:text-slate-100
                    hover:bg-slate-400 transition duration-300'  variant='outline'>
...
export default EditChangeManagementRequest;